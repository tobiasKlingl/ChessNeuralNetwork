"""
Self-written neural network library version 1.8 (cupy version)
Use "python run_nn_1_8.py" to use this library
...
class CrossEntropyCost(object):
    ...
    def delta(a,y):
        ...
        #print("XXa.shape=",a.shape)
        #print("XXy.shape=",y.shape)
        diff=a-y
        #A=a.ravel()
        #Y=y.ravel()
        #Diff=diff.ravel()
        #for i in range(A.size):
        #    print("i,Ai,Yi,Diffi=",i,A[i],Y[i],Diff[i])
        #print("diff.shape=",diff.shape)
        return diff # Here we assume to use sigmoid for output layer!!!
...
class Network(object):
    def __init__(self, sizes, showTime = False):
        print("##########################################################")
        print("##",52*' ',"##")
        print("##  ******** NNlib Version 1.8 (cupy version) ********* ##")
        print("##",52*' ',"##")
        print("##########################################################")
        print("")
    ...
    def large_weight_initializer(self,GD):
        ...
        if(GD=="SMGD"): # needed for SMGD
            self.bias_v  =[ cp.random.rand(y, 1)/cp.sqrt(y) for y   in self.sizes[1:] ]
            self.weight_v=[ cp.random.randn(y,x)/cp.sqrt(x) for y,x in zip(self.sizes[1:],self.sizes[:-1]) ]
            self.bias_m  =[ cp.zeros((y, 1)) for y   in self.sizes[1:] ]                     
            self.weight_m=[ cp.zeros((y,x))  for y,x in zip(self.sizes[1:],self.sizes[:-1]) ]
        else: # needed Adam and Nadam
            self.bias_v  =[ cp.zeros((y, 1)) for y   in self.sizes[1:] ]
    ...
    def feedforward(self,a,debug=False):
        a=cp.ascontiguousarray(a.transpose())
    ...
    def SGD(self, training_data, max_epochs, n_epochs, number_eta_reductions, mini_batch_size, eta,
        ...
        for j in range(max_epochs):
            ...
            if monitor_training and j%10==0: # Only monitor training data every 10th training epoch to speed up things
                cost,accuracy,n_train = self.cost_and_acc(training_data, lmbda)
                self.training_cost.append(cost)
                self.training_accuracy.append(accuracy)
                print("Cost on training data: {}".format(cost))
                print("Accuracy on training data: {} / {} ( {} % )".format(accuracy, n_train,100*accuracy/(n_train)))
                if self.Time: print("time: {0}".format(timer() - start_timeit))
            if monitor_evaluation:
                cost,accuracy,n_eval = self.cost_and_acc(evaluation_data, lmbda)
                self.evaluation_cost.append(cost)
                self.evaluation_accuracy.append(accuracy)
                print("Cost on evaluation data: {}".format(cost))
                print("Accuracy on evaluation data: {} / {} ( {} % )".format(accuracy, n_eval,100*accuracy/(n_eval)))
                if self.Time: print("time: {0}".format(timer() - start_timeit))
                
            # Early stopping
            max_eval_accuracy = max(self.evaluation_accuracy[-n_epochs:])
            if j >= n_epochs:
                print("Max. evaluation accuracy in last",n_epochs,"epochs :",100.0*max_eval_accuracy/(n_eval),"%")
                print("Reference evaluation accuracy on epoch",j-n_epochs,":",100.0*self.evaluation_accuracy[j-n_epochs]/(n_eval),"%")
    ...
    def update_mini_batch_matrixApproach(self, mini_batch, mu, eta, lmbda, n):
        ...
        x = cp.ascontiguousarray(mini_batch[0].transpose())
        size=len(mini_batch[1])
        y = mini_batch[1].reshape(1882,size)
    ...
    def backprop_matrixApproach(self, activation, y):
        ...
        nabla_w[-1] = delta@cp.ascontiguousarray(activations[-2].transpose())
        ...
        for l in range(2, self.num_layers):
            ...
            nabla_w[-l] = delta@cp.ascontiguousarray(activations[-l-1].transpose())
    ...
    def cost_and_acc(self, data, lmbda):
        ...
        for i in range(len(a)):
            print("i=",i,": a,y=",a[i][:300],y[i][:300])
        
        results = [(cp.argmax(ai),cp.argmax(yi)) for (ai,yi) in zip(a,y)]
        print("results=",results)
        s=sum(int(x == y) for (x, y) in results)
        """
        y=data[1].ravel()
        a=a.ravel()
        ...
        #    print("i,ai,yi=",i,ai,yi)
        
        a=a[y!=0]
        y=y[y!=0]
        diff=cp.abs(a-y)
        s=float(cp.sum(diff<0.01))
        n=diff.size
        
        randmStart=random.randint(1,int(y.size)-20)
        a,y,diff=a.tolist(),y.tolist(),diff.tolist()
        for i in range(randmStart,randmStart+20):
            print("i= {0}. |av-yv| = |{1:.{digits}f} - {2:.{digits}f}| = {3:.{digits}f}".format(i,a[i],y[i],diff[i],digits=3))
        return cost,s,n
