...
@njit(cache=True)
def willPlayerBeInCheck(boardpositions, move, debug, noOutputMode):
...
def nextMove(boardpositions,boardInput,mD,Sizes,Weights,Biases,colored=False,debug=False,noOutputMode=False):
    ...
    if(len(outOfCheckMoves)>0):
        ...
        """
        for i,moveProb in enumerate(moveProbabilities):
            if moveProb<maxProb*0.9:
                if noOutputMode==False:
                    print("moveProbabilities=",np.round(moveProbabilities,3,helper))
                    print("probability to win with move",i,"is",moveProb,"<0.9*",maxProb,"of move",argMax,"! Reduce to 33%.")
                moveProbabilities[i]=moveProb/3.0
            elif moveProb<maxProb*0.93:
                if noOutputMode==False:
                    print("moveProbabilities=",np.round(moveProbabilities,3,helper))
                    print("probability to win with move",i,"is",moveProb,"<0.93*",maxProb,"of move",argMax,"! Reduce to 50%.")
                moveProbabilities[i]=moveProb/2.0
            elif moveProb>maxProb*0.98:
                if noOutputMode==False:
                    print("moveProbabilities=",np.round(moveProbabilities,3,helper))
                    print("probability to win with move",i,"is",moveProb,">0.98*",maxProb,"of move",argMax,"! Increase to 125%.")
                moveProbabilities[i]=moveProb*1.25
        """
