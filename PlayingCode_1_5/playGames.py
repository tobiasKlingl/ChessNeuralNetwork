...
def play(idxString,boardInput,Sizes,Weights,Biases,boardState,mD,debug,noOutput,col):
    ...
    while(boardState.Finished==0):
        ...
        boardState.Finished,moveID=functions.nextMove(boardState,boardInput,mD,Sizes,Weights,Biases,col,debug,noOutput)
...
def playTheGames(startPly,Sizes,Weights,Biases,boardpositions,mD,deb,noOut,mergeInfo,c):
...
def runSelfPlay(boardpositions,mD,it,jobNum,startPly,nGames,Sizes,Weights,Biases,deb,noOut,mergeInfo,c):
    ...
    if(startPly>0):
        ...
        for i,bp in enumerate(boardpositions):
            ...
            bp.reconstructGameState(i,pieceBoardPositions[0],startPly)#.astype(np.int64)
            bp.nextPly()
    else:
        for i,bp in enumerate(boardpositions):
            bp.GameMode=gameMode
            bp.initializeBoard(i,c,deb,noOut)
    inputPos,inputID,currentGameID,gameWinners,gameCounters,exitState=playTheGames(startPly,Sizes,Weights,Biases,boardpositions,mD,deb,noOut,mergeInfo,c)
...
for startPly in range(upToPly):
    if exitState==0:
        start_timeit = timer()
        exitState=runSelfPlay(boardpositions,mD,iteration,jobNum,startPly,nGames,Sizes,Weights,Biases,Debug,NoOutput,PrintMergeInfo,Col)
