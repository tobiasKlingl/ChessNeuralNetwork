...
import copy
import sys
sys.path.append("../NetworkCode_1_8/")
import NNlib_1_8 as nn
...
def play(idx,boardState):
    ...
    if noOutputMode==False: print(idxString+": INFO: initialPlyNumber,initialPlayer,initialOpponent=",initialPlyNumber,initialPlayer,initialOpponent)
...
if("Selfplay" in gameMode):
    ...
    boardpositions=[bp.BoardPositions() for i in range(numberOfGamesToPlay)]
    if(plyToStart>0):
        bestPosToStart = open("data/bestLastMoves.pkl", "rb")
        pieceBoardPositions = pickle.load(bestPosToStart)
        for bp in boardpositions:
            bp.setGameMode(gameMode)
            bp.reconstructGameState(pieceBoardPositions[0],plyToStart)
            bp.nextPly()
    else:
        for bp in boardpositions:
            bp.initializeBoard(colored,debug,noOutputMode)
    gamePositions,gamePositionIDs,winners=[],[],[]
    exitState=0
    if useMultiprocessing:
        pool=mp.Pool()
        count=0
        result=pool.starmap_async(play,enumerate(boardpositions))
        ...
        for idx in range(numberOfGamesToPlay):
            gamePos,gameID,winner,finishedInOne=play(idx,boardpositions[idx])
            if winner!=-1:
    ...
    IDs=merged_data[2]
    posToWrite=[]
    idWinPerc=[(ID,round(num,4)) for ID,num in zip(IDs,winners)]
    for idWin in idWinPerc:
        print("ID,win percentage=",idWin[0]+":",idWin[1])
    for i in range(1):
